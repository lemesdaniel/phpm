...
fn parses_content_hash() {
    let lock = parse_lock(MINIMAL).expect("should parse");
...
fn parses_first_package_dist_and_source() {
    ...
    let dist = pkg.dist.as_ref().expect("has dist");
    assert_eq!(dist.dist_type, "zip");
    assert_eq!(dist.reference, "abc123");

    let source = pkg.source.as_ref().expect("has source");
